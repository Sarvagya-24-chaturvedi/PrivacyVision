...
export function startMutationWatch(
  engine: UnifiedPrivacyEngine,
  onDetection: (detections: SensitiveDetection[]) => void
): () => void {
  if (_observer) {
  ...
  _observer = new MutationObserver((mutations) => {
    const hasRelevantMutations = mutations.some((m) => {
      if (m.type === 'childList') {
        ...
        for (const node of m.addedNodes) {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            if (
              el.matches('input, textarea, select, form, [contenteditable]') ||
      ...
      if (m.type === 'attributes') {
        ...
        if (['type', 'name', 'placeholder', 'aria-label', 'value'].includes(m.attributeName || '')) return true;
      }
      return false;
    });
...
export function stopMutationWatch(): void {
  _observer?.disconnect();
  _observer = null;
}
