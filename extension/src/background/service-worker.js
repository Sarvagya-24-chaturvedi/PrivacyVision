...
async function sendToPage(tabId, message) {
    try {
        ...
        try {
            ...
            throw new Error(`PrivacyVision cannot inspect this tab. Open an http(s) page and refresh it after loading the extension. ` +
...
async function redactImageOffscreen(dataUrl, boxes, dpr) {
...
async function captureFullContext() {
    ...
    const domResponse = await sendToPage(tab.id, { type: "EXTRACT_DOM" });
    if (!domResponse?.ok) {
    ...
    const redactionResult = await redactImageOffscreen(rawScreenshot, domResponse.detections || [], domResponse.context.viewport?.dpr || 1);
    ...
    if (message.type === "RUN_AGENT") {
        (async () => {
            try {
                ...
                const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
                const execResponse = await sendToPage(tab.id, {
                    type: "EXECUTE_ACTION",
