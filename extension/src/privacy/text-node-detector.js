...
export class TextNodeDetector {
    static MAX_NODES = 2_000;
    static MAX_DETECTIONS = 150;
    static scanRenderedText() {
        const detections = [];
        ...
        for (const root of roots) {
            ...
            let node;
            while ((node = walker.nextNode()) && visited++ < this.MAX_NODES) {
                const parent = node.parentElement;
                const text = node.nodeValue || "";
                if (!parent || !text.trim() || /^(SCRIPT|STYLE|NOSCRIPT|TEXTAREA|OPTION)$/i.test(parent.tagName))
                    continue;
                ...
                for (const match of RegexDetector.scanText(text)) {
                    ...
                    if (detections.length >= this.MAX_DETECTIONS)
                        return detections;
                }
            }
        }
