import { BoundingBox, SensitiveDetection } from "../shared/privacy";
import { detectFacesOnCanvas } from "../vision/blazeface-runner";

export class FaceDetector {
  public static async detectFacesInDocument(): Promise<SensitiveDetection[]> {
    const detections: SensitiveDetection[] = [];
    const images = Array.from(
      document.querySelectorAll("img, [role='img'], canvas, svg[data-face], video")
    );
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      const isLikelyCandidateSemantically =
        /face|avatar|profile|user.?photo|headshot|portrait|team.?member|author|photo|person|selfie|pic/i.test(descriptor) ||
      ...
      const isLikelyUnlabelledPhoto = el.tagName === "IMG" &&
        (el as HTMLImageElement).naturalWidth >= 96 &&
        (el as HTMLImageElement).naturalHeight >= 96 &&
      ...
      if (el.tagName === "IMG" || el.tagName === "CANVAS") {
        try {
          ...
          ctx.drawImage(el as CanvasImageSource, 0, 0, offCanvas.width, offCanvas.height);
