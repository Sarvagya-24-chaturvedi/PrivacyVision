import { ActionExecutionResult, AgentAction } from "../shared/actions";

export class ActionExecutor {
  /**
   * Executes a locally validated browser agent action.
   */
  public static async execute(action: AgentAction): Promise<ActionExecutionResult> {
      ...
      switch (action.action) {
        case "CLICK": {
          ...
          if (!targetEl && typeof action.target.x === "number" && typeof action.target.y === "number") {
            targetEl = document.elementFromPoint(action.target.x, action.target.y) as HTMLElement;
          }
        ...
        case "FOCUS": {
          const el = document.querySelector(
            `[data-agent-id="${CSS.escape(action.target.agentId || "")}"]`
          ) as HTMLElement;
        ...
        case "SCROLL": {
          ...
          if (action.direction === "DOWN") deltaY = action.amount;
          if (action.direction === "UP") deltaY = -action.amount;
          if (action.direction === "RIGHT") deltaX = action.amount;
          if (action.direction === "LEFT") deltaX = -action.amount;
        ...
        default:
          throw new Error(`Unsupported action execution: ${(action as any).action}`);
      }
    } catch (err: any) {
      return {
        success: false,
