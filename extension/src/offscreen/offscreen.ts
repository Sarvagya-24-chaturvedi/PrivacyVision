import { RedactImageRequest, RedactImageResponse } from "../shared/messages";
import { ImageRedactor } from "../privacy/redactor";

chrome.runtime.onMessage.addListener((message: RedactImageRequest, _sender, sendResponse) => {
  ...
  (async () => {
    ...
    try {
      ...
      const redactionCount = ImageRedactor.redactCanvas(
        ctx,
        canvas.width,
        canvas.height,
        message.boxes || [],
      ...
      const response: RedactImageResponse = {
        ok: true,
        sanitizedDataUrl,
        redactionCount,
        durationMs
