...
import { BoundingBox, SensitiveDetection } from "../src/shared/privacy";

describe("Bounding Box Merger & IoU Fusion Layer", () => {
  it("should calculate correct IoU for disjoint boxes", () => {
    const b1: BoundingBox = { x: 0, y: 0, width: 100, height: 50 };
    const b2: BoundingBox = { x: 200, y: 200, width: 100, height: 50 };
    expect(BBoxMerger.computeIoU(b1, b2)).toBe(0.0);
  });

  it("should calculate correct IoU for identical boxes", () => {
    const b1: BoundingBox = { x: 50, y: 50, width: 100, height: 100 };
    const b2: BoundingBox = { x: 50, y: 50, width: 100, height: 100 };
    expect(BBoxMerger.computeIoU(b1, b2)).toBe(1.0);
  });

  it("should calculate correct IoU for partially overlapping boxes", () => {
    const b1: BoundingBox = { x: 0, y: 0, width: 100, height: 100 };
    const b2: BoundingBox = { x: 50, y: 0, width: 100, height: 100 };
  ...
  it("should preserve strongest redaction action during fusion", () => {
    const maskDet: SensitiveDetection = {
      id: "mask_1",
      type: "EMAIL",
      confidence: 0.75,
      sources: ["DOM"],
    ...
    const blackoutDet: SensitiveDetection = {
      id: "blackout_1",
      type: "PASSWORD",
      confidence: 0.98,
      sources: ["REGEX"],
  ...
  it("should handle nested boxes gracefully", () => {
    const outer: SensitiveDetection = {
      id: "outer",
      type: "CREDIT_CARD",
      confidence: 0.9,
      sources: ["DOM"],
    ...
    const inner: SensitiveDetection = {
      id: "inner",
      type: "CVV",
      confidence: 0.95,
      sources: ["REGEX"],
